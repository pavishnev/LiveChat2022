...
public signIn(login: Login): Observable<JWTtoken> {
    return this.httpClient.post<JWTtoken>(`${this.apiUrl}/auth/sign-in`, login)
    .pipe(
      tap(info => {
 ...
 public signUp(register: Register): Observable<JWTtoken> {
   //console.log(register)
    return this.httpClient.post<JWTtoken>(`${this.apiUrl}/auth/sign-up`, register)
    .pipe(
      tap(info => {
        this.initStorageInfo(info);
      //  this.decodeUserToken();
...
public signUpAgentComplete(obj:AgentRegisterComplete):Observable<JWTtoken>{
  return this.httpClient.post<JWTtoken>(`${this.apiUrl}/auth/sign-up-agent-complete`,obj) 
     .pipe(
      tap(info => {
        this.initStorageInfo(info);
      //  this.decodeUserToken();
  ...
  public signOut(): void {
   ...
   public get isUserAuthorizedGetter(): boolean {
    ...
    this.isUserAuthorized.next(token!=undefined && !this.jwtHelper.isTokenExpired(token));
