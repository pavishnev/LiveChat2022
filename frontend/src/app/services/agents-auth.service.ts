...
export class AgentsAuthService {
  ...
  public getAllAgents(): Observable<AgentsTableElement[]> {
    return this.httpClient.get<AgentsTableElement[]>(
      `${this.adminPageApiUrl}/get-all-agents`
    );
  }

  public getAgentCredentials(id: string): Observable<AgentsTableElement> {
    return this.httpClient.get<AgentsTableElement>(
      `${this.adminPageApiUrl}/get-agent-credentials/`,
      { params: { id: id } }
    );
  }

  public signUpAgent(obj: AgentRegister): Observable<UserId> {
    let baseUrl = window.location.origin + '/#/complete-registration/';
    obj.baseUrl = baseUrl;
    return this.httpClient.post<UserId>(
      `${this.authApiUrl}/sign-up-agent/`,
      obj
    );
  }
  public deleteAgent(id: string): Observable<void> {
    ...
    return this.httpClient.delete<void>(
      `${this.adminPageApiUrl}/delete-agent/${id}`
    );
  }
}
