    ...
    public class ChatHub : Hub
    {
        ...
        public async Task SendMessageToAgent(MessageModel msg)
        {
            ...
            try
            {
                ...
                {
                    await Clients.User(userId).SendAsync("Receive", new MessageModel() { SessionId = userId, IsSentByClient=false, Text= "Sorry, there is no available agents now. Please, try again later", Timestamp=DateTime.Now}) ;
                }
            }
            catch (Exception ex)
            {
